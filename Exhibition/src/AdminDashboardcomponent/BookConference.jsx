...
import { selectCOnferenceStaff } from '../redux/features/ConferenceSlice';
export default function BookConference({setopenConferenceStaffModal,setOpenConferenceByDate,selectConferenceId}) {
  const dispatch = useDispatch();
  const [staffChoiceIndex,setStaffChoiceIndex]=useState()
  const staffData = useSelector((state) => state?.conferenceReducer?.fetchConferenceStaffData);
  const selectData = useSelector((state) => state?.conferenceReducer?.selectConferenceStaffData);
useEffect(()=>{
  ...
  const selectStaff=()=>{
    ...
    dispatch(selectCOnferenceStaff(dataId))
  }
  return (
