...
import { AddProductInBrand, brandRegisterInExhibition, fetchAllBrandExhibitionInfo, FetchExhibitionForBrand, setbrandAddDataProduct } from '../redux/features/BrandSlice';
...
export default function BrandDashborad() {
  ...
  console.log(brandExhibitionData)
  const addProduct=()=>{
  ...
  const handleOpenModalProductInfo=(index)=>{
    setProductData(brandExhibitionData?.ProductData.filter((item)=>item.brandId===brandExhibitionData?.conferencedata[index]?.BrandId))
  ...
  const registerInConference=(index)=>{
    dispatch(brandRegisterInExhibition(exhibitionDataForCategorie?.conferenceData[index]._id))
  }
  const handleChangeParticularExhibition=()=>{
  ...
  <div className="border-[5px] border-blue-600 rounded-lg overflow-hidden shadow-lg ">
    {brandExhibitionData && brandExhibitionData?.AllBrandExhibitionData.map((item, index1) => {
      return (
        <div key={index1} className="relative m-[7px] bg-white rounded-lg shadow-md hover:shadow-lg h-[28vh]">
          <div className="relative h-[48%]">
            <img className="w-full h-[100%] object-cover rounded-t-lg opacity-70" src={item?.exhibitionBannerImg} alt="Exhibition" />
          ...
          <div className="">
            <div className="flex justify-around mt-[4px] text-sm text-gray-700">
              <div className='flex'>
                ...
                <p className='m-0'>{new Date(item?.exhibitionDuration?.Start).toLocaleDateString()}</p>
              </div>
              <div className='flex'>
                <p className="m-0 font-semibold">End Date:</p>
                <p className='m-0'>{new Date(item?.exhibitionDuration?.End).toLocaleDateString()}</p>
              </div>
            </div>
            <div className="flex">
              <p className="m-0 font-semibold">Exhibition Address:</p>
              <p className='m-0'>{item?.exhibitionAddress?.address}, {item?.exhibitionAddress?.city}, {item?.exhibitionAddress?.state}, {item?.exhibitionAddress?.pincode}</p>
            </div>
          </div>
          {brandExhibitionData?.conferencedata.map((conference, index2) => {
            console.log(index2)
            if (item._id === conference.exhibitionId) {
              return (
              ...
              <input type='button' onClick={()=>handleOpenModalAddProduct()} className="w-[11%] bg-blue-600 text-white rounded-[7px] hover:bg-blue-700 hover:cursor-pointer text-center" value="Add Product"/>
              {brandExhibitionData?.ProductData.length!==0&&<input type='button' onClick={()=>handleOpenModalProductInfo(index2)} className="w-[11%] bg-blue-600 text-white rounded-[7px] hover:bg-blue-700 hover:cursor-pointer text-center" value="Product Info"/>}
            </div>
                </div>
              )
            }
    ...
    <Modal.Header closeButton>
      ...
      <div className=' p-[1`%] overflow-y-auto rounded-lg overflow-hidden bg-white shadow-lg'>
        {exhibitionDataForCategorie?.exhibitionData.map((item, index)=>{return(
          <div className='flex border-[2px]  border-gray-400 rounded-lg justify-center items-center flex-col'>
            <h3 className='m-0'>{item?.exhibitionName}</h3>
            ...
            <div className=' w-[100%] flex justify-center h-[6vh] items-center'>
              <button onClick={()=>handleChangeParticularExhibition()} className=' w-[40%] h-[78%] rounded hover:bg-blue-500  bg-blue-600 text-[13px] text-[antiquewhite] '>All Exhibition Info</button>
            </div>
          </div>
        )})}
      </div>:
      (exhibitionDataForCategorie.msg==="The exhibition admin has not created any conferences yet."
      ?<div className="w-full max-w-md mx-auto mt-2">
      <p className='text-[17px] text-gray-500'>The exhibition admin has not created any conferences yet.</p>
      </div>:
      <div className="space-y-6">
      {exhibitionDataForCategorie?.exhibitionData.map((item, index) => (
        <div key={index} className="border-gray-400 rounded-lg overflow-hidden bg-white shadow-lg">
          <div className="relative">
            <img className="w-full h-64 object-cover brightness-50" src={item?.exhibitionBannerImg} alt={item?.exhibitionName} />
            <h3 className="absolute inset-0 flex justify-center items-center text-white font-extrabold text-2xl">{item?.exhibitionName}</h3>
          </div>
          ...
          <div className="p-4 space-y-4">
            <div className="flex justify-between text-sm text-gray-700">
              <div>
                ...
                <p>{new Date(item?.exhibitionDuration?.Start).toLocaleDateString()}</p>
              </div>
              <div>
                <p className="font-semibold">End Date:</p>
                <p>{new Date(item?.exhibitionDuration?.End).toLocaleDateString()}</p>
              </div>
            </div>

            <div className="space-y-2">
              <p className="font-semibold">Exhibition Address:</p>
              <p>{item?.exhibitionAddress?.address}, {item?.exhibitionAddress?.city}, {item?.exhibitionAddress?.state}, {item?.exhibitionAddress?.pincode}</p>
            </div>

            {exhibitionDataForCategorie?.conferenceData.length > 0 && (
              <div className="border-t pt-4 space-y-4">
                <h4 className="font-semibold text-lg text-gray-800">Conference Details</h4>
                {exhibitionDataForCategorie?.conferenceData.map((conference, index) => (
                  ...
                  <div className=' w-[100%] flex justify-center h-[6vh] items-center'>
                    <button onClick={()=>{registerInConference(index)}} className=' w-[40%] h-[78%] rounded hover:bg-blue-500  bg-blue-600 text-[13px] text-[antiquewhite] '>Register Your Brand In Exhibition</button>  
                    </div>
                  </div>
                ))}
              </div>
