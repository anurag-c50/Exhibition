    ...
    <>
        <div className='w-full flex justify-around  flex-col items-center bg-white rounded-lg shadow-lg'>
            {ProductData&&ProductData.map((item)=>{return(
                ...
                {item?.offerInfo?.isOffer&&<div className='flex flex-col'>
                    <label className='text-sm ftext-lg font-medium text-gray-800' htmlFor='productPrice'>Product on Shell</label>
                </div>}
                {item?.offerInfo?.isOffer&&<div className='flex flex-col'>
                    <label className='text-sm ftext-lg font-medium text-gray-600' htmlFor='productPrice'>Product Discount:</label>
                    <p id='productDescription' className=' m-0 text-base text-gray-700'>
                       {item?.offerInfo?.offerPercentage}%
                    </p>
                </div>}
                <div className='flex flex-col'>
